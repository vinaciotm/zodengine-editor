...
class SkyMesh extends Mesh {
	...
	constructor() {
		...
		/**
		 * Ground color shown below the horizon (vec3 RGB 0–1).
		 * @type {UniformNode<vec3>}
		 */
		this.groundColor = uniform( new Vector3( 0.18, 0.36, 0.54 ) );
		...
		const colorNode = /*@__PURE__*/ Fn( () => {
			...
			If( direction.y.lessThan( 0.0 ), () => {

				const groundBlend = clamp( direction.y.negate().mul( 5.0 ), 0.0, 1.0 );
				texColor.assign( mix( texColor, this.groundColor, groundBlend ) );
...
export { SkyMesh, SkyMesh as SkyMeshPatched };
